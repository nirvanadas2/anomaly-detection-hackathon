# Data Assumptions & Generation Logic
...
- **300 entities** over a **45-day window** starting `2026-06-10`.
- Random seed fixed at `42` (numpy Generator + Faker), so the run is
  reproducible byte-for-byte given the same code.
- This run produced **48008 events** total: **47349 normal**
  and **659 injected attack events** (**1.37%** of all
  events), within the requested 0.5%-3% injection-rate band
  (actual rate used this run: **1.41%**, drawn uniformly from
...
**1.41%**. Because the round-robin cycles evenly across six
