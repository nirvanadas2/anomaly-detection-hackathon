...
entity with fewer than {min_history} total events in the dataset -- and any
event that is among the *first* {window_minus_one} events of a
...
- a "typical resource set" (the smallest set of resources covering
  {top_resource_mass:.0%} of that entity type's access frequency),
- a "typical geo set" (same idea, {top_geo_mass:.0%} coverage),
...
window of {window_size} events, its events are scored by a sequence
...
2. An encoder LSTM consumes the {window_size}-event window and its final
...
3. A decoder LSTM re-expands that latent vector across all
   {window_size} timesteps, and a linear layer projects each timestep back
...
This run trained on **{n_windows_trained} windows** for
**{n_epochs_trained} epochs**, reaching a final training reconstruction MSE
of **{final_train_loss:.4f}**.

Of {total_events} total events across {n_entities} entities
({n_cold_start_entities} of them cold-start), **{n_scored_lstm}** were
scored by the LSTM and **{n_scored_fallback}** by the baseline fallback.
...
## 5. Concept drift: rolling baseline refresh every {drift_epoch_days} days
...
**recomputed every {drift_epoch_days} simulated days**, using only the
previous {trailing_days} days of data -- never the events they're about to
...
The very first {drift_epoch_days}-day period has no prior data to draw on,
